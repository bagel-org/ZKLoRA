...
mod tests {
    ...
    fn test_circuit_creation() {
        ...
        let expected_q = quantize_to_field(expected);
        let expected_output = vec![expected_q.magnitude];
        let expected_sign = vec![expected_q.sign];
        let prover = halo2_proofs::dev::MockProver::run(4, &circuit, vec![expected_output.clone(), expected_sign.clone()]).unwrap();
        assert!(prover.verify().is_ok());
        // Wrong output should fail
        let wrong_q = quantize_to_field(7.0);
        let wrong_output = vec![wrong_q.magnitude];
        let wrong_sign = vec![wrong_q.sign];
        let prover = halo2_proofs::dev::MockProver::run(4, &circuit, vec![wrong_output, wrong_sign]).unwrap();
    ...
    fn test_empty_inputs() {
        ...
        let expected_output = vec![expected_q.magnitude];
        let expected_sign = vec![expected_q.sign];
        let prover = halo2_proofs::dev::MockProver::run(4, &circuit, vec![expected_output, expected_sign]).unwrap();
        assert!(prover.verify().is_ok());
    }
    ...
    fn test_negative_inputs() {
        ...
        let expected_output = vec![expected_q.magnitude];
        let expected_sign = vec![expected_q.sign];
        println!("test_negative_inputs: input={} wa={} wb={} expected={} expected_quantized={:?} expected_decoded={} sign={:?}", input, wa, wb, expected, expected_q.magnitude, dequantize_from_field(expected_q), expected_q.sign);
        println!("test_negative_inputs: assigned input_sign={:?} weight_a_sign={:?} weight_b_sign={:?} output_sign={:?}", quantize_to_field(input).sign, quantize_to_field(wa).sign, quantize_to_field(wb).sign, expected_q.sign);
        let prover = halo2_proofs::dev::MockProver::run(4, &circuit, vec![expected_output, expected_sign]).unwrap();
        assert!(prover.verify().is_ok());
    }
