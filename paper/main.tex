...
\begin{abstract}
  ...
  We present \textbf{ZKLoRA}, a zero-knowledge verification protocol that relies on succinct proofs and multi-party inference to verify LoRA–base model compatibility without exposing LoRA weights. ZKLoRA produces \emph{deterministic} correctness guarantees and validates each LoRA module in \textbf{only 1--2 seconds} on state-of-the-art large language models. This low-latency approach enables nearly real-time verification and promotes secure collaboration among geographically dispersed teams and contract-based training pipelines. The protocol ensures that the delivered LoRA module works as claimed, safeguarding the contributor’s intellectual property while providing the base model user with verification of compatibility and lineage.
\end{abstract}
...
Overall, these plots confirm that \texttt{ZKLoRA} can handle large-scale implementation of LoRA modules with minimal overhead for verifying correctness, emphasizing the viability of repeated or multi-adapter scenarios in large-scale LLM pipelines.
...
\begin{tikzpicture}[
    ...
    arrow/.style={->, thick},
    % Define a style for the vertical arrow labels
    arrowlabel/.style={
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node[stepbox] (step1) {%
  \footnotesize \textbf{Step 1: Multi-Party Inference}\\[6pt]
  \vspace{10pt}
};

% Sub-blocks within Step 1
\node[subblock,
      label={[font=\scriptsize]above:{Base Model User}},
      anchor=center, 
      xshift=-1.6cm,
      yshift=-1.0cm] (bmuser) at (step1.center)
{};
...
\node[subblock,
      ...
      anchor=center, 
      xshift=+1.6cm,
      yshift=-1.0cm] (lorablk) at (step1.center)
{};

% Horizontal arrows in Step 1 (unchanged)
\draw[arrow] 
...
\draw[arrow] 
  ([yshift=-0.25cm]lorablk.west) 
    to[bend right=20] 
  node[midway, below, yshift=-0.05cm, font=\scriptsize]{LoRA Acts}
  ([yshift=-0.25cm]bmuser.east);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Step 2 BOX (Proof Generation)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node[stepbox, fill=orange!15, below=of step1] (step2) {%
  \footnotesize \textbf{Step 2: Proof Generation}\\[6pt]
  \footnotesize LoRA Contributor\\
  Builds Zero-Knowledge Proofs
};

% Vertical arrow from Step 1 to Step 2 with new style
\draw[arrow, shorten >=8pt, shorten <=8pt] (step1.south)
  -- node[arrowlabel, left, xshift=-0.1cm]{LoRA input/output acts}
  (step2.north);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Step 3 BOX (Verification)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\node[stepbox, fill=blue!10, below=of step2] (step3) {%
  \footnotesize \textbf{Step 3: Verification}\\[6pt]
  \footnotesize Base Model User\\
  Verifies each proof
};

% Vertical arrow from Step 2 to Step 3 with new style
\draw[arrow, shorten >=8pt, shorten <=8pt] (step2.south)
  -- node[arrowlabel, left, xshift=-0.1cm]{proof files}
  (step3.north);
