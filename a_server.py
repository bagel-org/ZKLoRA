#!/usr/bin/env python3
"""
User A: LoRA server for split inference & synchronous proof generation (no separate event loop thread).

Main fix:
 - We do `proof_res = asyncio.run(generate_proofs(...))` directly in the same handle_conn,
   so the 'end_inference' response is only sent AFTER proofs finish. B won't get EOF prematurely.
 - We also do config.use_cache = False to avoid 'past_key_values' errors.
 - We only expose submodules that truly have LoRA parameters & end with c_attn (you can adapt).
"""
